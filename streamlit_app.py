...
# Manage goals: add to timeline or remove
...
if st.button("Calculate Retirement Net Worth"):
    ...
    remaining_contributions = monthly_income - monthly_expenses - sum(goal['monthly_contribution'] for goal in st.session_state.goals if goal['added_to_timeline'])
    ...
    def plot_timeline():
        ...
        events = {
            'Year': [current_year, retirement_year] + [goal['target_year'] for goal in st.session_state.goals if goal['added_to_timeline']],
            'Event': ['Current Age', 'Retirement Age'] + [goal['name'] for goal in st.session_state.goals if goal['added_to_timeline']],
            'Text': [
                ...
                f"<b>Goal:</b> {goal['name']}<br><b>Amount Needed:</b> ${goal['amount_needed']:,.2f}<br><b>Monthly Contribution:</b> ${goal['monthly_contribution']:,.2f}" for goal in st.session_state.goals if goal['added_to_timeline']
            ]
        }
        ...
        fig.add_trace(go.Scatter(
            x=[current_year, retirement_year] + [goal['target_year'] for goal in st.session_state.goals if goal['added_to_timeline']], 
            y=[0] * (2 + len(st.session_state.goals)),
            mode='markers', 
            marker=dict(size=12, color='red', line=dict(width=2, color='black')), 
            text=['Current Age', 'Retirement Age'] + [goal['name'] for goal in st.session_state.goals if goal['added_to_timeline']],
